...
@implementation NSData (DopGZip)

+ (id)dopCompressedDataWithBytes:(const void *)bytes length:(unsigned)length
{
...
+ (id)dopCompressedDataWithData:(NSData *)data
{
   return [self dopCompressedDataWithBytes:[data bytes] length:(unsigned int)[data length]];
}


+ (id)dopDataWithCompressedBytes:(const void*)bytes length:(unsigned)length
{
   z_stream strm;
   int ret;
...
+ (id)dopDataWithCompressedData:(NSData *)compressedData
{
   return [self dopDataWithCompressedBytes: [compressedData bytes] length: (unsigned int)[compressedData length]];
}
